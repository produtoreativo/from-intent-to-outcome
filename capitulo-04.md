...
graph TD
    ...
    BS --> D
    BS -.->|"Assessment\ninicia antes de Discovery"| AP
    AP -.->|"qualifica\ncontexto"| D
    AP -.->|"qualifica\ncontexto"| DEL
    AR -.->|"lê o ciclo\nencerrado"| OP
    OP -->|"retroalimenta\ncom novos Signals"| BS
```
...
Na prática, um Business Signal frequentemente carrega características de mais de um Origin Stream: uma migração de infraestrutura pode ter implicações de conformidade; uma nova feature de produto pode depender de evolução técnica; uma automação de processo interno pode reduzir custo operacional com impacto indireto no cliente. Isso não é um problema e não exige que o Signal seja dividido. A classificação em um único Origin Stream não apaga as características dos outros; ela define o **ponto de entrada** pelo qual o Framework guiará o autor e o time na transição do Business Signal para o Business Intent. Cada Origin Stream ativa um conjunto distinto de perguntas, contextos e critérios de sucesso que orientam a investigação: um Signal Business pergunta sobre valor de mercado e comportamento do usuário; um Signal Enterprise pergunta sobre conformidade e rastreabilidade; um Signal Team pergunta sobre fluxo de trabalho e experiência do engenheiro; um Signal Technology pergunta sobre métricas técnicas e critérios de confiabilidade. Classificar corretamente é garantir que a investigação comece pelas perguntas certas.
